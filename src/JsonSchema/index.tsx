...
const defaultDataString =
  '{"title":"json-schema","$schema":"https://json-schema.org/draft-04/schema#","type":"object","properties":{},"required":[],"checked":false,"description":""}';
...
const JsonSchema: React.FC<Props> = props => {
  ...
  const SchemaArray = (prefix: string[], data: any, index: number) => {
    ...
    const handleAddChildField = () => {
      ...
      const json = util.addChildFieldAction(state, [
        ...prefixArray,
        'properties',
      ]);
      setState({
      ...
      <div key={index}>
        <Row gutter={2} className="json-schema__row">
            ...
            <Row>
              ...
              <Col span={22}>
                <FieldInput
                  defaultValue="Items"
                  disabled
                  addonAfter={<div style={{ width: '16px' }}></div>}
                  maxLength={64}
                />
              </Col>
            </Row>
          ...
          <Col span={2}>
            {items.type === 'object' ? (
              <PlusOutlined
                className="json-schema__row-add"
                onClick={handleAddChildField}
              />
            ) : null}
          </Col>
        </Row>
        <div className="children-item">{mapping(prefixArray, items)}</div>
      </div>
    );
  };
  ...
  const SchemaItem = (
    ...
    const handleChangeName = (e: any) => {
      ...
      let openKeys = [...keys, value, 'properties'].join(
        util.JSONPATH_JOIN_CHAR,
      );
      let oldOpenKeys = [...keys, name, 'properties'].join(
        util.JSONPATH_JOIN_CHAR,
      );
    ...
    const handleDeleteItem = () => {
      ...
      setState({
        checked: state.checked,
        data: util.enableRequireAction(state, {
          keys: prefixArray,
          name,
          required: false,
        }),
    ...
    const handleChangeValue = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      setState({
        checked: state.checked,
        data: util.changeValueAction(
          state,
          [...prefixArray, 'description'],
          value,
        ),
      });
    };
          ...
          <Menu.Item>
            <span
              onClick={() => {
                setState({
                  checked: state.checked,
                  data: util.addChildFieldAction(state, [
                    ...prefix,
                    name,
                    'properties',
                  ]),
      ...
      <div key={index}>
        <Row gutter={2} className="json-schema__row">
          ...
          <Col span={2}>
            ...
            <CloseOutlined
              className="json-schema__row-del"
              onClick={handleDeleteItem}
            />
          </Col>
        </Row>
        <div className="children-item">{mapping(prefixArray, value)}</div>
      </div>
    );
  };
  ...
  const SchemaObject = (perfix: string[], data: any) => {
    ...
    return (
      <div className="json-schema__object-item">
        {Object.keys(data.properties).map((c, index) =>
          SchemaItem(perfix, c, data, randomString(12)),
        )}
      </div>
    );
  };
            ...
            <Col span={22}>
              <Input
                ...
                addonAfter={
                  <Tooltip
                    placement="top"
                    title={state.checked ? '全部不必须' : '全部必须'}
                  >
                    <Checkbox
